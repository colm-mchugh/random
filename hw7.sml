...
datatype geom_exp = 
	 ...
	 | Var of string
	 | Shift of real * real * geom_exp (* deltaX, deltaY, e *)
...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx, dy, e) => let val x = eval_prog(e, env) in
				case x of
				    NoPoints => x
				 |  Point p => let val (x,y) = p in Point(x+dx, y+dy) end
				 |  Line l => let val (m, b) = l in Line(m, b + dy - m * dx) end
				 |  VerticalLine v => let val (vx) = v in VerticalLine(vx + dx) end
...
fun preprocess_prog (e) =
  case e of
   ...
   |  _ => e (* These remaining cases require no computation: NoPoints, Point, Line, VerticalLine, Var *)
